...
\leavevmode\\
    ...
    Let $j = 1$. \\
    \\
    At the beginning of the $1^{\text{st}}$ iteration, $\texttt{sum} = 0$ and $i = 0$. \\
    \\
    As well,
    \begin{equation*}
        \begin{aligned}
            \texttt{sum} &= \sum_{k=0}^{0-1} A[k] \\
            &= \sum_{k=0}^{-1} A[k] \\
            &= 0\text{.}
        \end{aligned}
    \end{equation*}
    ...
    Assume that for some iteration $m \in \{ 1, ..., len(A) \}$ \\
    \\
    Suppose $j \in \{ 1, \dots, len(j) \}$ and $Q(j)$ holds. \\
    \\
    \underline{Induction Step:} \\
    Proceed through the $j^{\text{th}}$ iteration as follows: \\
    \\
    Let $\texttt{sum}_{j}, i_{j}$ be values at the start of the $j^{\text{th}}$ iteration,
    \\
    conclusiongoeshere \\
\end{proof}
