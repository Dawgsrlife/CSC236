...
\leavevmode\\
    ...
    \\
    \underline{Case $(q = q_0, z = 0)$:} \\
    Assume $P_{q_0}(w)$ is true. Then, $0 \equiv w \mod{3}$. \\
    It follows that $P_{\delta(q, z)}(wz) = P_{\delta(q_0, 0)}(w0) = P_{q_0}(w0)$. Notice that, in treating the string $wz = w0$ as a binary integer representation, concatenating $0$ to $w$ performs a logical left shift (a multiplication by 2). Let $W$ be the integer representation of the string $w$; $2W$ shall be the integer representation of the string $wz$. By $P_{q_0}(w)$, $3 \mid W$, so $\exists n \in \mathbb{N}$ such that $W = 3n$. Perform the multiplication by $2$ and notice, $2W = 2 \times 3n = 3(2n)$. Declare that $\exists m \in \mathbb{N}$ such that $2W = 3(2n) = 3m$, meaning $3 \mid 2W$. It has been clearly demonstrated that $0 \equiv 2W \mod{3}$, where $2W$ is the integer representation of $wz$. Thus, $P_{\delta(q, z)}(wz) = P_{q_0}(w0)$ is true. \\
    \\
    \underline{Case $(q = q_0, z = 1)$:} \\
    Likewise, assume $P_{q_0}(w)$ is true. Then, $w = 0$. \\
    It follows that $P_{\delta(q, z)}(wz) = P_{\delta(q_0, 1)}(01) = P_{q_2}(01)$, which is clearly true by definition. \\
