...
\leavevmode\\
    ...
    \]
    For each state $q_i$, $i \in \{0\}$, in $\mathcal{D}$, define a state invariant $P_q(x)$, for strings $x \in \Sigma^* = \{0, 1\}^*$:
    \[
    ...
    \\
    Demonstrate that all state invariants hold when processing strings from the alphabet $\Sigma = \{0, 1\}$. \\
    ...
    \\
    By definition, $P_{q_0}(x): \text{$x$ is $\epsilon$ or consists of $0$s and $1$s}$. \\
    If $x \in L$, then it is either an empty string or consists of $0$s and $1$s. Clearly, $P_{q_0}(x)$ must be true. \\
...
\leavevmode\\
    ...
    \]
    For each state $q_i$, $i \in \{\epsilon, 0, 1, 2, 3, 4, 5\}$, in $\mathcal{D}$, define a state invariant $P_q(x)$ for strings $x \in \Sigma^* = \{0, 1\}^*$:
    \begin{equation*}
            \begin{aligned}
            ...
            \end{aligned}
    \end{equation*}
    It is clear that $q_i$ for $i \in \{\epsilon, 0, 1, 2, 3, 4\}$ are mutually exclusive by their definitions. The state, $q_5$, is the direct complement of the union of the other states, so $q_5$ mutually exclusive as well. Thus, all states in $\mathcal{D}$ are mutually exclusive. \\
    For exhaustivity, notice that every string in $\Sigma^* = \{0, 1\}^*$ is either $\epsilon$ or consists of some number of $0$s and $1$s. $q_\epsilon$ accounts for the empty string case, $q_i$ for $i \in \{0, 1, 2, 3, 4\}$ account for specific strings of $0$s and $1$s, and $q_5$ accounts for all the remaining cases of $0$s and $1$s not covered by the $q_i$. Thus, the states in $\mathcal{D}$ are exhaustive. \\
    \\
    ...
    Let $q \neq q_0$. $P_q(w) = P_{q_i}(\epsilon) \mid_{i \in \{0, 1, 2, 3, 4, 5\}}$ are trivially true as these corresponding states have not yet been reached. \\
    \\
    \underline{Induction Hypothesis:} \\
    ...
    This is achievable by showing that if $x \in \Sigma^* = \{0, 1\}^*$ is arbitrary, $x$ is a member of $L$ if and only if there exists an accepting state $q \in F$ such that $P_q(x)$ holds. \\
    \\
    By the invariants of the accepting states:
    \begin{equation*}
        \begin{aligned}
            P_{q_\epsilon}(x)&: \text{$x$ is the empty string, } \epsilon \\
            P_{q_0}(x)&: \text{$x$ is $0$} \\
        ...
        \end{aligned}
    \end{equation*}
    If $x \in L = \Sigma^* \setminus \{01, 101, 010\}$, then it is either an empty string or consists of $0$s and $1$s, but must not be either of $01$, $101$, nor $010$. Choose $q_{\epsilon}$ to cover the empty string case, and $q_5$ to cover all cases \textbf{except} $x \in \{0, 1, 10, 01, 010, 101\}$ Since $x$ must not be a member of $\{01, 010, 101\}$, it remains to account for $x \in \{0, 1, 10\}$. Conveniently, these cases are covered by $P_0$, $P_1$, and $P_3$. Thus, no matter which $x \in L = \Sigma^* \setminus \{01, 101, 010\}$, there exists accepting states $q \in F$ such that $P_q(x)$ holds. \\
    Conversely, $L = \Sigma^* \setminus \{01, 101, 010\}$ can be constructed by the same choices of accepting states. Choose the accepting states $q_i \in F, i \in \{\epsilon, 0, 1, 3, 5\}$ and assume $P_{q_i}(x)$ are true. Likewise as provided in the \textit{implication} proof, reconstruct $L = \Sigma^* \setminus \{01, 101, 010\}$ using the accepting states $P_{q_i}(x)$ for this \textit{implied-by} proof. It is clear that the accepting state's invariants build a language consisting of the empty string, and any combination of $0$s and $1$s, excluding the specific strings $01$, $101$, and $010$. Thus, if there are accepting states $q \in F$ such that $P_q(x), x \in \Sigma^* = \{0, 1\}^*$ holds, $x \in L = \Sigma^* \setminus \{01, 101, 010\}$. \\
    \\
